...
func sortCities(cities *TabArray, n int) {
	if n == 1 {
		return // Basis rekursi, array dengan 1 elemen sudah terurut
	}
	for i := 0; i < n-1; i++ {
		if cities[i].Distance > cities[i+1].Distance {
			cities[i], cities[i+1] = cities[i+1], cities[i]
		}
	}
	// Panggil rekursif untuk elemen yang tersisa
	sortCities(cities, n-1)
}